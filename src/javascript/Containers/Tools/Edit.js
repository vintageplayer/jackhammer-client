...
class Create extends Component {
  ...
  componentDidMount() {
    axiosInstance
      .get("/tools/" + this.props.match.params.id)
      .then(response => {
        this.setState({name: response.data.name, selectedScanType: response.data.scanTypeId, selectedLanguage: response.data.languageId})
      });
    this
      .props
      .actions
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.updateResponse) {
      ...
      toastr.success('Tool updated successfully');
    }
    if (nextProps.error) {
...
const mapStateToProps = (state) => ({fetchedLanguages: state.languages.fetchedLanguages, fetchedScanTypes: state.scanTypes.fetchedScanTypes, updateResponse: state.tools.updateResponse, error: state.tools.error,});
