...
class BugsClosingTrendChart extends Component {
  componentWillReceiveProps(nextProps) {
    const dashboardData = nextProps.dashboardData;
    if (dashboardData) {
      const bugsClosingTrend = dashboardData.bugsClosingTrend;
      var vulnerabilityTrendResult = bugsClosingTrend.vulnerabilityTrendResult;
      var criticalCount = [];
      var highCount = [];
      var mediumCount = [];
      var lowCount = [];
      ...
      for (var i = 11; i >= 0; i--) {
        ...
        lastTwelveMonths.push(monthList[month]);
      }
      lastTwelveMonths
      ...
      Highcharts.chart('bugsClosingChart', {
        colors: [
          'rgb(255, 77, 77)',
          'rgb(255, 112, 77)',
          'rgb(247, 163, 92)',
        ...
        title: {
          text: 'Vulnerability Closing Trend'
        },
        yAxis: {
          title: {
  ...
  render() {
    return (
      <div id="bugsClosingChart"></div>
    );
  }
}

const mapStateToProps = (state) => ({dashboardData: state.dashboard.dashboardData});
export default connect(mapStateToProps)(BugsClosingTrendChart);
